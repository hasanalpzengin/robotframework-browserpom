...
- `browserpom-create-pageobjects`: guidance for implementing PageObject and UIObject classes and exposing Robot keywords.
- `browserpom-cli`: discover available PageObjects and UIObjects before writing tests or adding components.

After installation, ask the LLM to use the relevant skill for the task. Use the discovery skill first when the available PageObjects or UIObjects are unknown. Use the create-pageobjects skill when implementing or consuming those objects.

## Configure Discovery

The discovery skill requires discovery configuration in the project's `pyproject.toml`:
...
For CLI-specific configuration, error messages, and interpretation notes see `skills/browserpom-cli/SKILL.md` — the CLI prints helpful errors when discovery configuration is missing or invalid which agents should use to guide fixes.
...
6. Ask the agent to use the create-pageobjects skill when creating PageObjects, nested UIObjects, custom keywords, or Robot Framework integrations.
