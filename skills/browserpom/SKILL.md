...
# BrowserPOM Project Usage (Create Robot Tests)

Use this skill when creating, modifying, or consuming Python PageObjects and UIObjects with Robot Framework Browser.

## CLI Discovery

Before choosing or reusing an existing PageObject or UIObject, use the companion "browserpom-create-pageobjects" skill to discover available classes in the project. The CLI reads the `pyproject.toml` configuration and outputs class names, source paths, and public methods for each existing PageObject or UIObject.

Note on skill references: other skills commonly reference the skill name that should be called. For example, to reference the discovery skill use the skill path `/browserpom-create-pageobjects` (see similar usage in other skill repositories).
